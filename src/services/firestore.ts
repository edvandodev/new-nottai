...
export const firestoreService = {
  listenToClients: (callback: (clients: Client[]) => void) => {
    ...
    (async () => {
      callbackId = await FirebaseFirestore.addCollectionSnapshotListener<Omit<Client, 'id'>>(
        { reference: CLIENTS },
        (event, error) => {
          if (cancelled) return;
          if (error || !event) {
          ...
          const clients = event.snapshots.map(s => ({ ...(s.data ?? {}), id: s.id } as Client));
          callback(clients);
        }
      );
    })();
  ...
  listenToSales: (callback: (sales: Sale[]) => void) => {
    ...
    (async () => {
      callbackId = await FirebaseFirestore.addCollectionSnapshotListener<Omit<Sale, 'id'>>(
        { reference: SALES },
        (event, error) => {
          if (cancelled) return;
          if (error || !event) {
          ...
          const sales = event.snapshots.map(s => ({ ...(s.data ?? {}), id: s.id } as Sale));
          callback(sales);
        }
      );
    })();
  ...
  listenToPayments: (callback: (payments: Payment[]) => void) => {
    ...
    (async () => {
      callbackId = await FirebaseFirestore.addCollectionSnapshotListener<Omit<Payment, 'id'>>(
        { reference: PAYMENTS },
        (event, error) => {
          if (cancelled) return;
          if (error || !event) {
          ...
          const payments = event.snapshots.map(s => ({ ...(s.data ?? {}), id: s.id } as Payment));
          callback(payments);
        }
      );
    })();
  ...
  listenToPriceSettings: (callback: (settings: PriceSettings | null) => void) => {
    ...
    (async () => {
      callbackId = await FirebaseFirestore.addCollectionSnapshotListener<PriceSettings>(
        { reference: SETTINGS },
        (event, error) => {
          if (cancelled) return;
          if (error || !event) {
